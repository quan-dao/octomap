...
void print_query_info(point3d query, EvidOcTreeNode* node) {
  if (node != NULL) {
    ...
    cout << "evid mass of node is: " << node->getMass() << endl;
		// cout << "----------------------------------------------------------";
  }
  else 
...
int main(int argc, char** argv) {
  ...
  std::string filename ("simple_evid_tree.ot");
  std::cout << "Writing evid tree to " << filename << std::endl;
  // write evid tree
  EXPECT_TRUE(tree.write(filename));


  // read tree file
  cout << "Reading evid tree from "<< filename <<"\n";
  ...
  EvidOcTree* read_evid_tree = dynamic_cast<EvidOcTree*>(read_tree);
  EXPECT_TRUE(read_evid_tree);

  EXPECT_TRUE(tree == *read_evid_tree);
    ...
    EvidOcTreeNode* result2 = read_evid_tree->search(query);
    std::cout << "READ: ";
    ...
    EXPECT_TRUE(result);
    EXPECT_TRUE(result2);
    // EXPECT_EQ(result->getMass(), result2->getMass());
    EXPECT_EQ(result->getLogOdds(), result2->getLogOdds());
    
    query = point3d(-1.,-1.,-1.);
    result = tree.search(query);
    result2 = read_evid_tree->search(query);
    print_query_info(query, result);
    std::cout << "READ: ";
    print_query_info(query, result);
    std::cout << "WRITE: ";
  ...
  // delete / create some nodes
  {
    ...
    EvidOcTreeNode* newNode = tree.updateNode(newCoord, true);
    ...
    EvidOcTreeNode* parentNode = tree.search(newCoord, tree.getTreeDepth() -1);
    ...
    EXPECT_TRUE(tree.write("simple_evid_tree_ed.ot"));
